...
Feito para a pasta que passou do ponto de assistir tudo de novo — uma grade de
frames costuma bastar para decidir, e os vídeos para os quais ela não basta
ganham uma pilha própria.
...
- Uma sessão gráfica (Linux, macOS ou Windows)
...
A interface fala **português (Brasil)** e **inglês**. O seletor fica no topo
direito da tela inicial e troca na hora — sem reiniciar, e uma sessão de revisão
em andamento sobrevive à troca. A escolha fica salva no `config.json`; sem nada
salvo, o programa segue o locale do sistema e usa inglês como reserva.

O idioma também governa o que não é texto: o separador decimal (`1.234,5 GB`
contra `1,234.5 GB`), o formato de data e o nome padrão das pastas de descartes e
de *talvez* (`_para_apagar` / `_talvez` em português, `_to_delete` / `_maybe` em
inglês). Como são diretórios no disco, os nomes dos dois idiomas são sempre
reconhecidos como "ainda é o padrão" — trocar de idioma nunca deixa órfã uma
pasta que você já tem.
...
| `config.json` | `~/.config/video_manager/` | idioma, últimas pastas, opções usadas e atalhos |
...
├── ui_shortcuts.py  diálogo de personalização das teclas
└── app.py         janela principal
```

## Contribuindo

Issues e pull requests são bem-vindos. Duas coisas que vale saber antes de mexer
no código:

- **Nenhum texto visível ao usuário vai direto para um módulo de UI.**
  Acrescente uma chave em `i18n.py`, com os dois idiomas, e chame
  `tr("sua.chave")`. Uma chave sem tradução aparece crua na tela, de propósito —
  um fallback silencioso esconderia a omissão.
- As telas que podem ser redesenhadas implementam `retranslate()`, que reescreve
  todo rótulo fixo. Um widget novo com texto precisa de uma linha lá, senão ele
  fica congelado no idioma em que foi criado.
